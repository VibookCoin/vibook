...
#include "ui_zvibookcontroldialog.h"
...
std::list<std::string> ZViBOOKControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZViBOOKControlDialog::listMints;

ZViBOOKControlDialog::ZViBOOKControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZViBOOKControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZViBOOKControlDialog::~ZViBOOKControlDialog()
{
    delete ui;
}

void ZViBOOKControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZViBOOKControlDialog::updateList()
{
...
void ZViBOOKControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZViBOOKControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZViBOOK_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZViBOOKControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZViBOOKControlDialog::GetSelectedMints()
{
...
void ZViBOOKControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
