...
#include "ui_zbookcontroldialog.h"
...
std::list<std::string> ZBOOKControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBOOKControlDialog::listMints;

ZBOOKControlDialog::ZBOOKControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBOOKControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBOOKControlDialog::~ZBOOKControlDialog()
{
    delete ui;
}

void ZBOOKControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBOOKControlDialog::updateList()
{
...
void ZBOOKControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBOOKControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBOOK_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBOOKControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBOOKControlDialog::GetSelectedMints()
{
...
void ZBOOKControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
