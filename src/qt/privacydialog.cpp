                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zBOOK ought to be enough for anybody." - Bill Gates, 2017
    ui->zBOOKpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzBOOKSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzBOOK->setEnabled(false);
        ui->pushButtonMintzBOOK->setToolTip(tr("zBOOK is currently disabled due to maintenance."));

        ui->pushButtonSpendzBOOK->setEnabled(false);
        ui->pushButtonSpendzBOOK->setToolTip(tr("zBOOK is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zBOOKpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzBOOK_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBOOK is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zBOOK, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBOOK...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBOOK in ") +
...
void PrivacyDialog::on_pushButtonSpendzBOOK_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBOOK is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zBOOK, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zBOOK
        sendzBOOK();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBOOK
    sendzBOOK();
}

void PrivacyDialog::on_pushButtonZBOOKControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZBOOKControlDialog* zBOOKControl = new ZBOOKControlDialog(this);
    zBOOKControl->setModel(walletModel);
    zBOOKControl->exec();
}

void PrivacyDialog::setZBOOKControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBOOKSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBOOK()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid ViBOOK Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zBOOKpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zBOOK is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BOOK </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBOOK</b>";
    ...
    // use mints from zBOOK selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBOOKControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZBOOKControlDialog::GetSelectedMints();
    }
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zBOOK was spent successfully update the addressbook with the label
    ...
    // Clear zbook selector in case it was used
    ZBOOKControlDialog::listSelectedMints.clear();
    ui->labelzBOOKSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBOOK Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " BOOK, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zBOOK Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBOOK </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zBOOK "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zBOOK "));
    ui->labelzBOOKAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zBOOK </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zBOOK </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzBOOKSyncStatus->setVisible(fShow);
}
