...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the BOOK and zBOOK money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 22262, 35417));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in BOOK/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in BOOK/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, vibook, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in BOOK/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-backupzbook=<n>", strprintf(_("Enable automatic wallet backups triggered after each zBOOK minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizevibookamount=<n>     " + strprintf(_("Keep N BOOK anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The ViBOOK Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //ViBOOK specific: zerocoin and spork DB's
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZBOOKMinted();
                        RecalculateZBOOKSpent();
                    }
                    RecalculateBOOKSupply(1);
                }
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        bool fEnableZBOOKBackups = GetBoolArg("-backupzbook", true);
        pwalletMain->setZBOOKAutoBackups(fEnableZBOOKBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    if (nAnonymizeViBOOKAmount < 2) nAnonymizeViBOOKAmount = 2;
    ...
    /* Denominations
       ...
       For example:
       1BOOK+1000 == (.1BOOK+100)*10
       10BOOK+10000 == (1BOOK+1000)*10
    */
