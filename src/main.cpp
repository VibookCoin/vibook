...
#if defined(NDEBUG)
#error "ViBOOK cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ubook) are considered zero fee (for relaying and mining)
...
bool ContextualCheckCoinSpend(const CoinSpend& spend, CBlockIndex* pindex, const uint256& txid)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
            return error("%s : zBOOK with serial %s is already in the block %d\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zBOOK spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckCoinSpend(spend, chainActive.Tip(), txid))
                    return state.Invalid(error("%s: zBOOK spend in tx %s failed to pass context checks", __func__, txid.GetHex()));
            }
        } else {
            LOCK(pool.cs);
...
int64_t GetBlockValue(int nHeight)
{
    int64_t nSubsidy = 0;
		...
		} else if (nHeight <= 525600 && nHeight > 125) {
		nSubsidy = 999 * COIN;
			...
			nSubsidy = 600 * COIN;
		} else if (nHeight <= 4783000 && nHeight > 4730400) {
			nSubsidy = 550 * COIN;
		} else if (nHeight <= 5308600 && nHeight > 4783000) {
			nSubsidy = 500 * COIN;
		} else if (nHeight <= 5834200 && nHeight > 5308600) {
			nSubsidy = 450 * COIN;
		} else if (nHeight <= 6359800 && nHeight > 5834200) {
			nSubsidy = 400 * COIN;
		} else if (nHeight <= 6885400 && nHeight > 6359800) {
			nSubsidy = 350 * COIN;
		} else if (nHeight <= 7411000 && nHeight > 6885400) {
			nSubsidy = 300 * COIN;
		} else if (nHeight <= 7936600 && nHeight > 7411000) {
			nSubsidy = 250 * COIN;
		} else if (nHeight <= 8462200 && nHeight > 7936600) {
			nSubsidy = 200 * COIN;
		} else if (nHeight <= 8987800 && nHeight > 8462200) {
			nSubsidy = 150 * COIN;
		} else if (nHeight <= 9513400 && nHeight > 8987800) {
			nSubsidy = 100 * COIN;
		} else if (nHeight <= 10039000 && nHeight > 9513400) {
			nSubsidy = 100 * COIN;
		} else if (nHeight <= 10564600 && nHeight > 10039000) {
			nSubsidy = 50 * COIN;
		} else if (nHeight <= 11090200 && nHeight > 10564600) {
			nSubsidy = 50 * COIN;
		} else {
		nSubsidy = 0.1 * COIN;
		}
...
int64_t GetTreasuryAward(int nHeight)
{
	if(IsTreasuryBlock(nHeight)) {
		if(nHeight == nStartTreasuryBlock)
			return 200 * COIN; //200 for the first treasury block
		else
			return 144150 * COIN; //144150 for each next block
	} else
		return 0;
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from ViBOOK
...
{
    RenameThread("vibook-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZBOOKMinted()
{
...
void RecalculateZBOOKSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zBOOK supply
...
bool RecalculateBOOKSupply(int nHeightStart)
{
...
bool UpdateZBOOKSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateBOOKSupply(Params().Zerocoin_StartHeight());
    }

    //Track zBOOK money supply in the block index
    if (!UpdateZBOOKSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zBOOK supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zBOOKSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert BOOK to zBOOK
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zBOOK spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zBOOK serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zBOOK mints and %d zBOOK spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // ViBOOK: We use certain sporks during IBD, so check to see if they are
